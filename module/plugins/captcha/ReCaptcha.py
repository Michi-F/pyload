...
class ReCaptcha(CaptchaService):
    ...
    def _challenge_v2(self, key, secure_token=None):
        ...
        if re.search(r'href="https://support.google.com/recaptcha/\?hl=\w+?#6223828"', html) is not None:
            self.log_warning(_("reCAPTCHA noscriprt is blocked, trying reCAPTCHA interactive"))
    ...
    def _challenge_v2js(self, key, secure_token=None):
        ...
        try:
            ...
            filename = "captcha_request_%s_%s.nofile" % (self.pyfile.plugin.__name__, time_ref)
            # we don't have an actual image to decrypt -> pass the URL and the site-key for further use in plugins/hooks
            img = {'url': self.pyfile.url, 'sitekey': key}
            self.task = captchaManager.newTask(img, "interactive", filename , "interactive")
